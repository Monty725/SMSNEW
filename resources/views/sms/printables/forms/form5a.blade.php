<div id="form5a" style="break-after: page">
    ...
    @endphp
    <h4 class="no-margin"><b>SUGAR RELEASE ORDER (REF SRO) AND DELIVERY REPORT - REFINED</b> </h4>
...
{{--        ORIGINAL ORIGINAL ORIGINAL CODE--}}
        @if(!empty($wr->form5aIssuancesOfSro))
            @foreach($wr->form5aIssuancesOfSro as $form5aIssuancesOfSro)
                @if(is_null($form5aIssuancesOfSro->quedan_only) && is_null($form5aIssuancesOfSro->is_deleted_5a))
                    @php
                        $totals['rawQty'] = $totals['rawQty'] +
                            (($form5aIssuancesOfSro->raw_qty != 0) ? $form5aIssuancesOfSro->raw_qty : $form5aIssuancesOfSro->prev_raw_qty);

                        $totals['refinedQty'] = $totals['refinedQty'] +
                            (($form5aIssuancesOfSro->refined_qty != 0) ? $form5aIssuancesOfSro->refined_qty : $form5aIssuancesOfSro->prev_refined_qty);
                    @endphp
                    <tr>
                        ...
                        <td class="text-right">
                            {{ $form5aIssuancesOfSro->raw_qty != 0
                                ? number_format($form5aIssuancesOfSro->raw_qty, 2)
                                : number_format($form5aIssuancesOfSro->prev_raw_qty, 2) }}
                        </td>
                        <td>{{ $form5aIssuancesOfSro->monitoring_fee_or_no }}</td>
                        <td>{{ $form5aIssuancesOfSro->rsq_no }}</td>
                        <td class="text-right">
                            {{ !empty($form5aIssuancesOfSro->refined_qty)
                                ? number_format($form5aIssuancesOfSro->refined_qty, 2)
                                : number_format($form5aIssuancesOfSro->prev_refined_qty, 2) }}
                        </td>
                    </tr>
                @endif
            @endforeach
