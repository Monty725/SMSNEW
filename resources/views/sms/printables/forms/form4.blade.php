<div id="form4" style="break-after: page">
    ...
    <h4 class="no-margin"><b>MILL SITE & SUBSIDIARY WAREHOUSE INVENTORY REPORT - RAW</b></h4>
...
{{--            <td class="text-right">{{\App\Swep\Helpers\Helper::toNumber($total['stockBalances']['current'] + $stock_current)}}</td>--}}
{{--            <td class="text-right">{{\App\Swep\Helpers\Helper::toNumber($total['stockBalances']['prevCWeek'] + $stock_prevToDate_current)}}</td>--}}
            <td colspan="3" class="text-right">{{\App\Swep\Helpers\Helper::toNumber($total['stockBalances']['toCDate'] + $stock_toDate_current)}}</td>
{{--            <td class="text-right">{{\App\Swep\Helpers\Helper::toNumber($total['stockBalances']['prev'] + $stock_prev)}}</td>--}}
{{--            <td class="text-right">{{\App\Swep\Helpers\Helper::toNumber($total['stockBalances']['prevPWeek'] + $stock_prevToDate_prev)}}</td>--}}
            <td colspan="3" class="text-right">{{\App\Swep\Helpers\Helper::toNumber($total['stockBalances']['toPDate'] + $stock_toDate_prev)}}</td>
        </tr>
        <tr style="font-weight:600; height:50px;">
            <td style="width:400px;">
